...
En la parte superior izquierda se puede observar como la máquina que está corriendo es la clonada.
...
* minor#(x): número menor, por debajo del número mayor de manera jerárquica, se realizan cambios significativos dentro de la misma versión principal 
...
Primero, es necesario movernos a la carpeta del kernel, después se realiza una copia del archivo de configuración del kernel actual al archivo .config, el cual es un archivo oculto. 
```bash
cd linux-5.17.5
...
Y posterior a esto, realizar la modificación de las siguientes líneas al inicio:
